...
export interface IUseSub1NewReestrChangeRowParent {
  activeRowForChangeParent: Ref<INewTableRow>;
  onChangeRowParentId: (newRowParentId: number) => void;
};

export function useSub1NewReestrChangeRowParent(
  sub1Reestr: IUseSub1NewReestr,
): IUseSub1NewReestrChangeRowParent {
  const initialData = ref<INewTableRow[]>(sub1Reestr.data.value);
